...
plot_ly(arboles, labels = ~Especie, values = ~g_ha, type = "pie",
        textinfo = "label+percent", insidetextorientation = "radial")

## Calidad 
calidadExpan<-with(arboles,rep(Calidad,expan))
plot_ly(y=table(calidadExpan),x=names(table(calidadExpan)),type="bar",
        marker=list(color="lightgreen"))%>%
  layout(title=("Representacion de calidad"))

## Forma
formaExpan<-with(arboles,rep(Forma,expan))
plot_ly(y=table(formaExpan),x=names(table(formaExpan)),type="bar",
        marker=list(color="lightblue"))%>%
  layout(title="Representacion de forma")

## Origen
origenExpan<-with(arboles,rep(Origen,expan))
...
plot_ly(y=table(especiesExpan),x=names(table(especiesExpan)),type="bar",
  ...
  layout(title="Numero de arboles de cada especies")
#Representar el % de area basimetrica para cada una de las especies 
especiesg_ha<-arboles[,c("Especie","g_ha")]
espg_ha<-matrix(NA,ncol=2,nrow=length(unique(especiesg_ha$Especie)))
for(i in 1:length(unique(especiesg_ha$Especie))){
  aux<-especiesg_ha[especiesg_ha$Especie==unique(especiesg_ha$Especie)[i],]
  especie<-unique(aux$Especie)
  gha<-sum(aux$g_ha)
  gha<-round(gha,5)
  espg_ha[i,1]<-especie
  espg_ha[i,2]<-gha
...
espg_ha<-as.data.frame(espg_ha)
colnames(espg_ha)<-c("Especie","g_ha")

espg_ha$porcentaje<-with(espg_ha,(round(g_ha/sum(g_ha),4))*100)
espg_ha$Especie<-with(espg_ha,ifelse(porcentaje<1,"Otras",Especie))
plot_ly(espg_ha, labels =factor(espg_ha$Especie), values =espg_ha$g_ha, type = "pie",
        textinfo = "label+percent", insidetextorientation = "radial")
